...
=head1 OPTIONS
 ...
 -CCversion	CF3 or CF4 or CF5 (will the reads be duplicate filtered CC3 or CC4 or CC5 style ? )
...
=head1 AUTHOR
 ...
 and further to this release (CF5) by Jelena Telenius Nov 2017
...
(
	...
	"CCversion=s"=>\ $version,			# -CCversion	CF3 or CF4 or CF5 (will the reads be duplicate filtered CC3 or CC4 or CC5 style ? )
);
  ...
  #if (($chr =~ /chr(.*)/) and ($cigar =~/(\d++)(.*)/) and ($name =~ /(.*):PE(\d++):(\d++)$/)) # checks that the sam file matches the name, cigar string etc (non aligned reads will not do this)
  {
    ...
    # The rest of the reads are now parse-able :
	    ...
	    # Also normal - NON-flashed first fragment (2)
	      ...
	      push @{$data{$readname}{"coord array"}}, $data{$readname}{$pe}{$readno}{"chr"}.":".$data{$readname}{$pe}{$readno}{"readstart"}."-".$data{$readname}{$pe}{$readno}{"readend"}."-".$data{$readname}{$pe}{$readno}{"strand"};
	      $nonflashedcounters{"02 Fragment is First Fragment of R1 or R2, and has RestrictionEnzyme cut in the END of the read (fastq coordinates)"}++;
	    }	
	    # Special cases - certain reads of NON-FLASHED file. (islastfragment values 0,1 for non-flashed file)
	      ...
	      {
		# Push first, count second
		push @{$data{$readname}{"coord array"}}, $NEWstart."-".$NEWend."-".$data{$readname}{$pe}{$readno}{"strand"};
		
	        if ($islastfrag==1){
	        $nonflashedcounters{"01 Fragment is Last Fragment of R1 or R2, and has no RestrictionEnzyme cut in the END of the read (fastq coordinates)"}++;
	        }	
		else {
		# Rest of them have to be 0
		$nonflashedcounters{"00 Fragment has no RestrictionEnzyme cut in either side"}++;		
		}
	      }
...
for (my$i=0; $i< (scalar (@capturesite_data)); $i++)
{
  ...
  {
    wigtobigwig($outputfilename."_".$capturesite_data[$i][0], \*REPORTFH, $public_folder, $public_url, "CaptureC_gene_$capturesite_data[$i][0]");
    wigtobigwig($outputfilename."_".$capturesite_data[$i][0]."_win", \*REPORTFH, $public_folder, $public_url, "CaptureC_gene_$capturesite_data[$i][0]");
